...
setwd("/home/marie/portfolio/") # change file path accordingly
...
ui <- fluidPage(
  titlePanel(h2("Net Primary Production",align = "center")),
  sidebarLayout(    
    sidebarPanel(
      
      selectInput(inputId = "dataset",
                  label = "Choose a fire:",
                  choices = c("Montana_CrazyHorse_Fire_2003", "California_Rim_Fire_2013", "Arizona_Wallow_Fire_2011",
                              "NewMexico_Unnamed_Fire_2003", "California_Valley_Fire_2015",
                              "NewMexico_Bear_Fire_2006", "Oregon_Biscuit_Fire_2002"), 
                  selected = "Montana_CrazyHorse_Fire_2003")),
    mainPanel(
      plotOutput("ts_plot"),
...
server <- shinyServer(
  ...
  function(input,output){
    ...
    output$ts_plot <- renderPlot({
      ...
      dataset <- datasetInput()
      ggplot(dataset, aes(x = date, y=result)) + geom_point() +geom_line()
